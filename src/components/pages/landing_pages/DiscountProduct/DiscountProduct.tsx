...
const DiscountProduct: React.FC<ProductsProps> = ({ products }) => {
  const prevRef = useRef(null);
  const nextRef = useRef(null);

  console.log("find all discount product", products);
  ...
  return (
    <div className="Container pt-12 relative">
      ...
      <h2 className="md:text-2xl font-semibold md:text-center hidden md:block mt-2 uppercase">
        Discount Products
      </h2>
      <div className="flex justify-between">
        <p className="here using this p for arrow right site hidden md:block"></p>
        <h2 className="md:text-2xl font-semibold text-start md:hidden uppercase">
          Discount Products
        </h2>
